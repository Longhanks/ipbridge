    ...
    if not abs_path.exists():
        ...
        try:
            subprocess.check_call(
                ['/usr/bin/curl', '-L', image_url, '-o', abs_path]
            )
        except subprocess.CalledProcessError as e:
            current_app.logger.info(
                f'Downloading {image_url} failed! Error: {e}'
            )
            return None
...
def get_readinglist() -> List[ReadinglistItem]:
    ...
    readinglist = [
        e for e in data if e.get('Title') == 'com.apple.ReadingList'
    ][0]
    ...
    for item in readinglist_elements:
        ...
        date: datetime = item['ReadingList']['DateAdded'].replace(
            tzinfo=tz.tzutc()
        ).astimezone(tz.tzlocal())
        preview = item['ReadingList'].get('PreviewText')
        rl_item = ReadinglistItem(
            title=title,
            url=url,
            image_url=image_url,
            date=date,
    ...
    unsaved = cache.get(current_app.config['READING_LIST_UNSAVED_KEY']) or []

    # First, check if unsaved items are now part of the readinglist
    # and remove them from the unsaved list, if so.
    ...
    # Save cache updates.
    cache.set(
        key=current_app.config['READING_LIST_UNSAVED_KEY'],
        value=unsaved,
        timeout=120,
    )
    ...
    deleted: List[ReadinglistItem] = cache.get(
        current_app.config['READING_LIST_DELETED_KEY']
    ) or []
    ...
    for deleted_item_index, deleted_item in enumerate(deleted):
        ...
        for index, rl_item in enumerate(pythonic_readinglist):
            ...
            if (
                rl_item.url in (deleted_item.url, deleted_item.url + '/')
                and delta <= 1.5
            ):
                rl_index = index
                break
    ...
    # Remove the deleted items form the list that the readinglist finished
    # deleting.
    ...
    # Save cache updates.
    cache.set(
        key=current_app.config['READING_LIST_DELETED_KEY'],
        value=deleted,
        timeout=120,
    )
...
def add_readinglist_item(url: str):
    ...
    rl_item = ReadinglistItem(
        title=url, url=url, image_url=None, date=date_added, preview=url
    )
    current_app.logger.info(
        'New readinglist item: '
        + f'{{ "URLString ": "{url}", '
    ...
    unsaved = (
        cache.get(key=current_app.config['READING_LIST_UNSAVED_KEY']) or []
    )
...
def delete_readinglist_item(index: int):
    ...
    thread = threading.Thread(
        target=subprocess.check_call, args=(osascript_call,)
    )
    thread.start()
    deleted = (
        cache.get(key=current_app.config['READING_LIST_UNSAVED_KEY']) or []
